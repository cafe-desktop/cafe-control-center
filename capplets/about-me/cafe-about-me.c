...
about_me_update_photo (CafeAboutMe *me)
{
	...
	if (me->image_changed && me->have_image) {
		...
		if (do_scale) {
			...
			scaled = cdk_pixbuf_scale_simple (pixbuf, width*scale, height*scale, GDK_INTERP_BILINEAR);
...
about_me_setup_dialog (void)
{
	...
	if (ctk_builder_add_from_resource (dialog, "/org/cafe/ccc/am/cafe-about-me-dialog.ui", &error) == 0)
        {
