...
create_builder (void)
{
    ...
    if (ctk_builder_add_from_resource (builder, "/org/cafe/mcc/accessibility/at/at-enable-dialog.ui", &error)) {
        ...
        ctk_image_set_from_file (CTK_IMAGE (object),
        ...
        if (app_info == NULL) {
            ...
            ctk_widget_hide (CTK_WIDGET (object));
        } else {
...
close_logout_update (CtkBuilder *builder)
{
	...
	ctk_widget_set_sensitive (CTK_WIDGET (btn), initial_state != curr_state);
		  ...
		  CtkBuilder  *builder)
{
	...
	ctk_toggle_button_set_active (CTK_TOGGLE_BUTTON (btn),
				      is_enabled);
}
